...
const getCards = async () => {
  ...
  return fetch('https://api.magicthegathering.io/v1/cards?pageSize=5').then(a => a.json())
}

const getCard = async (name) => {
    console.log('fetching card: ', name)
    
    return fetch(`https://api.magicthegathering.io/v1/cards?name=${name.replace('-', ' ')}`).then(a => a.json())
}
